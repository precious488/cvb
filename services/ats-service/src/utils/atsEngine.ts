...
export interface ResumeForATS {
  personalInfo: { fullName: string; email: string; phone: string; location: string; title: string };
  summary: string;
  experience: Array<{ position: string; company: string; description: string; startDate: string; endDate: string; current: boolean }>;
...
function textOf(resume: ResumeForATS): string {
  return [
    resume.summary,
...
export function scoreResume(resume: ResumeForATS, jobDescription?: string): ATSResult {
  ...
  const formattingScore = Math.round(
    ([hasLinkedIn, hasMultipleSkills, hasProjects, sectionScores.contact === 100, !!resume.summary].filter(Boolean).length / 5) * 100
  );
  ...
  if (sectionScores.summary < 50) suggestions.push('Add a compelling professional summary (2–4 sentences) to pass initial screening.');
  if (actionVerbScore < 60) suggestions.push(`Use more action verbs in experience descriptions. Try: ${ACTION_VERBS.slice(0, 6).join(', ')}.`);
  if (quantScore < 40) suggestions.push('Add quantifiable achievements (e.g., "Reduced load time by 40%", "Led team of 5").');
  if (resume.skills.length < 6) suggestions.push('Expand your skills section — ATS systems scan for specific technical keywords.');
  if (missingKeywords.length > 0) suggestions.push(`Your resume is missing key terms from the job posting: ${missingKeywords.slice(0, 5).join(', ')}.`);
  if (sectionScores.contact < 100) suggestions.push('Complete your contact information (name, email, phone, location).');
  if (!hasProjects && resume.experience.length < 2) suggestions.push('Add projects to strengthen your profile.');
...
export function hashJobDescription(jd: string): string {
  return crypto.createHash('sha1').update(jd.trim().toLowerCase()).digest('hex').slice(0, 12);
}
